<?php
$page_title="Construction Trades";
 $program_title="Construction Trades";
 $videoId="";
 $replacementImg="../assets/construction/replace.jpg";
 $programOverview="Construction trades can lead a student into over 50 different career paths. These students learn teamwork among group members, problem solving and critical thinking, the ability to make decisions for clients, grow their physical strength and stamina, industry computer skills, hand eye coordination and dexterity, math and business skills, and an increased understanding of tool uses.";
 $isNewPathway = TRUE;

 $courseSeq1="Recommended";
 $courseSeq2="Year 1";
 $courseSeq3="Year 2 Block";
 $courseSeq4="Year 3";
 $courseSeq5=""; 

 $courseSeq1Class1="Introduction to Design Processes";
 $courseSeq1Class2="";
 $courseSeq2Class1="Principles of Construction Trades";
 $courseSeq2Class2="";
 $courseSeq3Class1="Construction Trades:  General Carpentry";
 $courseSeq3Class2="Construction Trades:  Framing and Finishing";
 $courseSeq4Class1="Construction Trades Capstone";
 $courseSeq4Class2="";
 $courseSeq5Class1="";
 $courseSeq5Class2=""; 

 
 $courseSeq1Class1Info="Introduction to Design Processes is a course that introduces all ETE career pathways and it focuses on creative problem solving in developing, testing, communicating, and presenting post-evaluation of products.";
 $courseSeq1Class2Info="";
 $courseSeq2Class1Info="Principles of Construction Trades prepares students with the basic skills needed to continue in a construction trade field."; 
 $courseSeq2Class2Info="";
 $courseSeq3Class1Info="Construction Trades: General Carpentry builds upon the skills learned in the Principles of Construction Trades and examines the pro- cedures for laying out and constructing floor systems, wall systems, ceiling joist and roof framing, and basic stair layout.";
 $courseSeq3Class2Info="Construction Trades: Framing and Finishing prepares students with advanced framing skills along with interior and exterior finishing techniques.";
 $courseSeq4Class1Info="Construction Trades Capstone will continue to build on students’ knowledge and skills in the construction field as they put their skills in knowledge to work by completing the building of a custom home that passes all inspections and is then put on the market to sell. Students must have instructor approval to enroll.";
 $courseSeq4Class2Info="";
 $courseSeq5Class1Info="";
 $courseSeq5Class2Info="";

 $grade1="Grades 9-12";
 $grade2="Grades 10-12";
 $grade3="Grades 11-12";
 $grade4="Grade 12";
 $grade5="";
 $teacher1="Mr. Ott";
 $teacher2="Mr. Gogel";
 $teacher3="Mr. Everett";
 $teacher4="Mr. Everett";
 $teacher5="";
 $prerequisites1="None";
 $prerequisites2="None";
 $prerequisites3="(1) Principles of Construction Trades";
 $prerequisites4="(1) rinciples of Construction Trades; (2) Construction Trades:  General Carpentry; (3) Construction Trades:  Framing and Finishing";
 $prerequisites5="";

 
 $galleryImgs=["../assets/construction/gallery1.jpg","../assets/construction/gallery2.jpg","../assets/construction/gallery3.jpg"];
 $certifications=[["Osha 10 Certification"],["NCCER Certification"]];
 $certificationsImg=["../assets/construction/construction1.jpg","../assets/construction/nccer.jpg"];
 $certificationsName=["Osha","NCCER"];
 $partners="";
 $partnersImg=[""];
 $partnersName=[""];
 $dualCredit="Students can earn up to 18 free dual credits in Construction Trades: 9 in Construction Trades I and 9 in Construction Trades II";
 $dualCreditImg="https://www.ivytech.edu/images/ivy-tech-2015-logo_header.png";
 $dualCreditName="Ivy Tech";
 $internships="";
 $internshipsImg="";
 $internshipsName="";
 $testimonies=[""];
 $testimoniesName=[""];
 $extraImgs=["../assets/construction/extra1.jpg","../assets/construction/extra3.jpg"];

 
 
 
 include($_SERVER["DOCUMENT_ROOT"] . '/program.php');
?>
